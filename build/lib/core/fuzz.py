...
async def fuzz_get_param(url, param, payload):
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=TIMEOUT)) as session:
        try:
            ...
            if similarity < SIMILARITY_THRESHOLD:
                gf_matches = run_gf_scan(fuzzed_url)
                nuclei_output = run_nuclei_scan(fuzzed_url)
                record = {
                    "url": fuzzed_url,
                    "parameter": param,
                    "payload": payload,
...
async def fuzz_post_param(url, param, payload):
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=TIMEOUT)) as session:
        try:
            baseline_data = {param: ""}
