...
class Scanner:
    ...
    async def enumerate_subdomains(self, domain: str) -> Set[str]:
        ...
        tasks = [
            ...
            run_tool(f"findomain -t {domain} --quiet"),
            run_tool(f"chaos -d {domain} -silent"),
            run_tool(f"dnsx -d {domain} -silent"),
            run_tool(f"sublist3r -d {domain} -n"),
            run_tool(f"curl -s 'https://crt.sh/?q=%.{domain}&output=json' | jq -r '.[].name_value' | sort -u")
        ]

        # Add shuffledns if resolvers.txt exists
        if os.path.exists("resolvers.txt"):
            tasks.append(run_tool(f"shuffledns -d {domain} -w wordlist.txt -r resolvers.txt"))
